...
void do_gouge( CHAR_DATA * ch, const char *argument )
{
   ...
   {
      dam = number_range( 1, ch->skill_level[COMMANDO_ABILITY] );
      ...
      if( global_retcode == rNONE )
      {
         if( !IS_AFFECTED( victim, AFF_BLIND ) )
         {
            ...
            af.modifier = -6;
            af.duration = 3 + ( ch->skill_level[COMMANDO_ABILITY] / 20 );
...
/*	    WAIT_STATE( ch, skill_table[gsn_detrap]->beats ); */
   ...
   percent = number_percent(  ) - ( ch->skill_level[INFILTRATOR_ABILITY] / 20 ) - ( get_curr_lck( ch ) - 16 );
...
void do_steal( CHAR_DATA * ch, const char *argument )
{
   ...
   if( !str_cmp( arg1, "credits" ) || !str_cmp( arg1, "credit" ) || !str_cmp( arg1, "money" ) )
   {
      ...
      if( IS_NPC( victim ) )
      {
         xp =
            UMIN( amount * 10,
                  ( exp_level( ch->skill_level[INFILTRATOR_ABILITY] + 1 ) -
                    exp_level( ch->skill_level[INFILTRATOR_ABILITY] ) ) / 35 );
   ...
   if( IS_NPC( victim ) )
   {
      xp =
         UMIN( obj->cost * 10,
               ( exp_level( ch->skill_level[INFILTRATOR_ABILITY] + 1 ) -
                 exp_level( ch->skill_level[INFILTRATOR_ABILITY] ) ) / 10 );
      xp = UMIN( xp, xp_compute( ch, victim ) );
      gain_exp( ch, xp, INFILTRATOR_ABILITY );
      ch_printf( ch, "&WYou gain %ld smuggling experience!\r\n", xp );
   }
...
void do_kick( CHAR_DATA * ch, const char *argument )
{
   ...
   if( IS_NPC( ch ) || number_percent(  ) < ch->pcdata->learned[gsn_kick] )
   {
      ...
      global_retcode = damage( ch, victim, number_range( 10, ( ( 10 + ch->skill_level[COMMANDO_ABILITY] ) * 2 ) ), gsn_kick );
   }
   else
   {
...
void do_punch( CHAR_DATA * ch, const char *argument )
{
   ...
   if( IS_NPC( ch ) || number_percent(  ) < ch->pcdata->learned[gsn_punch] )
   {
      ...
      global_retcode = damage( ch, victim, number_range( 20, ( ( 20 + ch->skill_level[COMMANDO_ABILITY] ) * 2 ) ), gsn_punch );
   }
   else
   {
...
void do_bash( CHAR_DATA * ch, const char *argument )
{
   ...
   if( IS_NPC( ch ) || ( number_percent(  ) + schance ) < ch->pcdata->learned[gsn_bash] )
   {
      ...
      global_retcode = damage( ch, victim, number_range( 1, ch->skill_level[COMMANDO_ABILITY] ), gsn_bash );
   }
   else
   {
      WAIT_STATE( ch, 2 * PULSE_VIOLENCE );
...
void do_disarm( CHAR_DATA * ch, const char *argument )
{
   ...
   percent = number_percent(  ) + victim->skill_level[COMMANDO_ABILITY] - ch->skill_level[COMMANDO_ABILITY]
...
void do_pick( CHAR_DATA * ch, const char *argument )
{
   ...
   {
      if( IS_NPC( gch ) && IS_AWAKE( gch ) && ch->skill_level[INFILTRATOR_ABILITY] < gch->top_level )
      {
...
void do_sneak( CHAR_DATA * ch, const char *argument )
{
   ...
   {
      af.type = gsn_sneak;
      af.duration = ( int )( ch->skill_level[INFILTRATOR_ABILITY] * DUR_CONV );
...
void do_poison_weapon( CHAR_DATA * ch, const char *argument )
{
   ...
   if( !IS_NPC( ch ) && percent > ch->pcdata->learned[gsn_poison_weapon] )
   {
      ...
      damage( ch, ch, ch->skill_level[HUNTER_ABILITY], gsn_poison_weapon );
   ...
   obj->cost *= ch->skill_level[HUNTER_ABILITY] / 2;
   /*
    * Set an object timer.  Don't want proliferation of poisoned weapons 
    */
   obj->timer = 10 + ch->skill_level[HUNTER_ABILITY];
...
void do_hitall( CHAR_DATA * ch, const char *argument )
{
   ...
   for( vch = ch->in_room->first_person; vch; vch = vch_next )
   {
      ...
      if( is_same_group( ch, vch ) || !is_legal_kill( ch, vch ) || !can_see( ch, vch ) || is_safe( ch, vch ) )
         continue;
      if( ++nvict > ch->skill_level[COMMANDO_ABILITY] / 5 )
         break;
      if( chance( ch, percent ) )
      {
         if( !IS_NPC( ch ) && !IS_NPC( vch ) )
