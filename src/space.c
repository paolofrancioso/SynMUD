...
void damage_ship_ch( SHIP_DATA * ship, int min, int max, CHAR_DATA * ch )
{
   ...
   xp = ( exp_level( ch->skill_level[DEFENDER_ABILITY] + 1 ) - exp_level( ch->skill_level[DEFENDER_ABILITY] ) ) / 25;
   xp = UMIN( get_ship_value( ship ) / 100, xp );
   gain_exp( ch, xp, DEFENDER_ABILITY );
   ...
   if( ship->hull <= 0 )
   {
      ...
      xp = ( exp_level( ch->skill_level[DEFENDER_ABILITY] + 1 ) - exp_level( ch->skill_level[DEFENDER_ABILITY] ) );
...
void do_land( CHAR_DATA * ch, const char *argument )
{
   ...
   if( number_percent(  ) < schance )
   {
      ...
      {
         int xp = ( exp_level( ch->skill_level[DEFENDER_ABILITY] + 1 ) - exp_level( ch->skill_level[DEFENDER_ABILITY] ) );
         xp = UMIN( get_ship_value( ship ), xp );
         gain_exp( ch, xp, DEFENDER_ABILITY );
...
void do_info( CHAR_DATA * ch, const char *argument )
{
   ...
   if( !str_cmp( target->owner, ch->name ) || !str_cmp( target->pilot, ch->name ) || !str_cmp( target->copilot, ch->name ) )
      ch_printf( ch, "&G| &WOwner: &w%s&G || &WPilot: &w%s&G || &WCoPilot: &w%s&G |\r\n",
                 target->owner != nullptr ? target->owner : "None.", target->pilot != nullptr ? target->pilot : "None.",
                 target->copilot != nullptr ? target->copilot : "None." );
