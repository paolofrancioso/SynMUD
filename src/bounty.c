      ...
      if( IS_SET( victim->act, PLR_KILLER ) && !IS_NPC( ch ) )
      {
         bexp =
            URANGE( 1, xp_compute( ch, victim ),
                    ( exp_level( ch->skill_level[HUNTER_ABILITY] + 1 ) - exp_level( ch->skill_level[HUNTER_ABILITY] ) ) );
         gain_exp( ch, bexp, HUNTER_ABILITY );
         set_char_color( AT_BLOOD, ch );
         ch_printf( ch, "You receive %ld hunter experience for executing a wanted killer.\r\n", bexp );
      }
      else if( !IS_NPC( ch ) )
      {
