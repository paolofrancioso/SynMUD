...
int max_level( CHAR_DATA * ch, int ability )
{
   ...
   if( ability == COMMANDO_ABILITY )
   {
      ...
      level += ( ch->perm_str / 4 ) + ( ch->perm_dex / 4 );

   }

   if( ability == DEFENDER_ABILITY )
   {
      if( ch->main_ability == ability )
         level = 25;
      if( ch->secondary_ability == ability )
      ...
      level += ( ch->perm_str / 4 ) + ( ch->perm_con / 4 );
   }
   ...
   if( ability == PERSUADER_ABILITY )
   {
      ...
      level += ch->perm_cha / 4 + ch->perm_wis / 4;
...
void obj_update( void )
{
   ...
   for( obj = last_object; obj; obj = gobj_prev )
   {
      ...
      if( obj->item_type == ITEM_WEAPON && obj->carried_by &&
          ...
          obj->value[3] != WEAPON_MINIGUN && obj->value[3] != WEAPON_PULSE_LASER && 
		  obj->value[3] != WEAPON_GRAVITON_GUN && obj->value[3] != WEAPON_SNIPER_RIFLE && 
		  obj->value[3] != WEAPON_FLAME_THROWER && obj->value[3] != WEAPON_LAUNCHER )
      {
         obj->value[4]--;
         if( obj->value[4] <= 0 )
         {
