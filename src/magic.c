...
void do_cast( CHAR_DATA * ch, const char *argument )
{
   ...
   else
   {
      ...
      else
      {
         ...
         retcode = ( *skill->spell_fun ) ( sn, ch->skill_level[COMMANDO_ABILITY], ch, vo );
         end_timer( &time_used );
   ...
   if( retcode != rSPELL_FAILED )
   {
      ...
      force_exp =
         URANGE( 0, force_exp,
                 ( exp_level( ch->skill_level[COMMANDO_ABILITY] + 1 ) - exp_level( ch->skill_level[FORCE_ABILITY] ) ) / 35 );
      if( !ch->fighting )
...
ch_ret spell_create_mob( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   mob->armor = interpolate( mob->top_level, 10, -15 );
