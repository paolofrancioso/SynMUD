...
bool spec_newbie_pilot( CHAR_DATA * ch )
{
   ...
   for( victim = ch->in_room->first_person; victim; victim = v_next )
   {
      ...
      switch ( victim->race )
      {
/*         case RACE_HUMAN: //Modify to Change Starting location by race
            home = 201;
      ...
      for( obj = victim->last_carrying; obj; obj = obj->prev_content )
      {
         if( obj->pIndexData->item_type == ITEM_SMUT )
         {
            if( victim != ch && can_see( ch, victim ) && can_see_obj( ch, obj ) )
            {
               ...
               ch_exp =
                  UMIN( obj->cost * 10,
                        ( exp_level( victim->skill_level[INFILTRATOR_ABILITY] + 1 ) -
                          exp_level( victim->skill_level[INFILTRATOR_ABILITY] ) ) );
               ch_printf( victim, "You lose %ld experience.\r\n ", ch_exp );
               gain_exp( victim, 0 - ch_exp, INFILTRATOR_ABILITY );
               return TRUE;
            }
            else if( can_see( ch, victim ) && !IS_SET( obj->extra_flags, ITEM_CONTRABAND ) )
            {
               ...
               gain_exp( victim, ch_exp, INFILTRATOR_ABILITY );
