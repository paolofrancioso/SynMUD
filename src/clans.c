...
bool load_clan_file( const char *clanfile )
{
   ...
   if( found )
   {
      ...
      {
         log_string( "...HINT: Storeroom not found" );
         return found;
      }
...
/*    if ( ch->speaking & LANG_CLAN )
   ...
   lose_exp = UMAX( ch->experience[PERSUADER_ABILITY] - exp_level( ch->skill_level[PERSUADER_ABILITY] ), 0 );
   ch_printf( ch, "You lose %ld diplomacy experience.\r\n", lose_exp );
   ch->experience[PERSUADER_ABILITY] -= lose_exp;
