...
void fread_planet( PLANET_DATA * planet, FILE * fp )
{
   ...
   for( ;; )
   {
      ...
      switch ( UPPER( word[0] ) )
      {
         ...
         case 'P':
		    KEY( "Population", planet->population, fread_number( fp ) );
...
void load_planets(  )
{
   ...
   for( ;; )
   {
	  ...
	  sprintf( buf, "%s%s", PLANET_DIR, filename );
	  log_string( buf );
...
void do_planets( CHAR_DATA * ch, const char *argument )
{
   ...
   for( starsystem = first_starsystem; starsystem; starsystem = starsystem->next )
       if ( str_cmp(starsystem->name, "Fake") != 0 )
...
SPACE_DATA *get_starsystem( const char *name )
{
   SPACE_DATA *starsystem;
   ...
   for( starsystem = first_starsystem; starsystem; starsystem = starsystem->next )
      if( nifty_is_name( name, starsystem->name ) )
         return starsystem;
   ...
   for( starsystem = first_starsystem; starsystem; starsystem = starsystem->next )
      if( nifty_is_name_prefix( name, starsystem->name ) )
         return starsystem;

   return NULL;
}

void do_makeregion( CHAR_DATA * ch, const char *argument )
{
   char filename[256];
   PLANET_DATA *planet;
   ...
   if( !argument || argument[0] == '\0' )
   {
      send_to_char( "Usage: makeregion <region name>\r\n", ch );
      return;
   }
   ...
   planet->next_in_system = NULL;
   planet->prev_in_system = NULL;
   planet->starsystem = get_starsystem( "Fake" );
   planet->first_area = NULL;
   planet->last_area = NULL;
   planet->first_guard = NULL;
   planet->last_guard = NULL;
   ...
   send_to_char( "Your task is not complete. Use SETREGION to modify Region Data.\r\n", ch );
   send_to_char( "Start with: setregion <region> filename <sample.zone>.\r\n", ch );
   send_to_char( "This will create the region ready to be used.\r\n", ch );
}

void do_setregion( CHAR_DATA * ch, const char *argument )
{
   char arg1[MAX_INPUT_LENGTH];
   char arg2[MAX_INPUT_LENGTH];
   PLANET_DATA *planet;
   ...
   if( arg1[0] == '\0' )
   {
      send_to_char( "Usage: setregion <region> <field> [value]\r\n", ch );
      send_to_char( "\r\nField being one of:\r\n", ch );
      send_to_char( " name filename area base_value population governed_by popsupport\r\n", ch );
      return;
   }

   if( !( planet = get_planet( arg1 ) ) )
   {
      send_to_char( "No such region.\r\n", ch );
      return;
   }
   ...
   if( !strcmp( arg2, "name" ) )
   {
      ...
      if( ( tplanet = get_planet( argument ) ) != NULL )
      {
         send_to_char( "A region/planet with that name already Exists!\r\n", ch );
         return;
      }
    ...
    if( !strcmp( arg2, "base_value" ) )
    {
        ...
        if( (bval < 1) || (bval > MAX_PLANET_BASE_VALUE) )
        {
           ch_printf( ch, "&YRegion base values range from &W%d &Yto &W%d&Y!\n\r", 1, MAX_PLANET_BASE_VALUE );
           return;
        }
        planet->base_value = bval;
        send_to_char( "Done.\n\r", ch );
   ...
   if( !strcmp( arg2, "population" ) )
   {
      planet->population = atoi( argument );
      send_to_char( "Done.\r\n", ch );
      save_planet( planet );
      return;
   }
   ...
   if( !strcmp( arg2, "filename" ) )
   {
      ...
      for( tplanet = first_planet; tplanet; tplanet = tplanet->next )
      {
          if( !str_cmp( tplanet->filename, argument ) )
          {
              send_to_char( "A region/planet with that filename already exists!\r\n", ch );
              return;
          }
      }
      ...
      planet->filename = str_dup( argument );
      send_to_char( "Done.\r\n", ch );
	  send_to_char( "Remember to hotboot the system to load the new region.\r\n", ch );
      save_planet( planet );
      write_planet_list(  );
      return;
   }
...
void do_regions( CHAR_DATA * ch, const char *argument )
{
   PLANET_DATA *planet;
   int count = 0;
   SPACE_DATA *starsystem;

   send_to_char( "&CRegion             Population    Governed By                  Popular Support\r\n", ch );
   ...
   for( starsystem = first_starsystem; starsystem; starsystem = starsystem->next )
      ...
      for( planet = starsystem->first_planet; planet; planet = planet->next_in_system )
      {
         if( planet->controls != 0 )
            continue;
         ch_printf( ch, "&c%-18s %-12d  %-25s    ",
                    planet->name, planet->population ? planet->population : 0, planet->governed_by ? planet->governed_by->name : "" );
         ch_printf( ch, "%.1f\r\n", planet->pop_support );
         if( IS_IMMORTAL( ch ) && !planet->area )
         {
            ch_printf( ch, "&RWarning - this region is not attached to an area!&G" );
            ch_printf( ch, "\r\n" );
         }
   ...
   {
      set_char_color( AT_BLOOD, ch );
      send_to_char( "There are no regions up to now.\r\n", ch );
   }
