...
void do_pcrename( CHAR_DATA * ch, const char *argument )
{
   ...
   rentitle = (char*)" was just renamed";
   char * title = (char *) malloc(1 + strlen(capitalize( arg2 ))+ strlen(rentitle) );
   strcpy(title, capitalize( arg2 ));
   strcat(title, rentitle);

   victim->pcdata->title = STRALLOC( title );
...
void do_mstat( CHAR_DATA * ch, const char *argument )
{
   ...
   ch_printf( ch,
              "&W&z|  &GStr&W: %-2d  &GInt&W: %-2d  &GWis&W: %-2d  &GDex&W: %-2d  &GCon&W: %-2d  &GCha&W: %-2d  &GLck&W: %-2d               &z|\r\n",
 ...
 *****/
   ...
   switch ( obj->item_type )
   {
      ...
      case ITEM_WEAPON:
         ...
         if( obj->value[3] == 0 )
         ...
         else if( obj->value[3] == WEAPON_VIBRO_AXE )
            ...
            ch_printf( ch, "&WFlame Thrower&W\r\n" );						
         else if( obj->value[3] == WEAPON_DEFL_SHIELD )
            ch_printf( ch, "&WDeflecting Shield&W\r\n" );
				 else if( obj->value[3] == WEAPON_BLUDGEON )
            ch_printf( ch, "&WBludgeon&W\r\n" );
         else
