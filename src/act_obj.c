...
void do_remove( CHAR_DATA * ch, const char *argument )
{
   ...
   if( obj->item_type == ITEM_CYBER_EYE
		...
		|| obj->item_type == ITEM_CYBER_EPIDERMIS )
   {
      send_to_char( "You cannot remove cyberware by yourself.\r\n", ch );
      return;
   }
   remove_obj( ch, obj->wear_loc, TRUE );
   return;
         ...
         switch ( obj->item_type )
         {
            ...
            case ITEM_WEAPON:
               ch_printf( ch, "&z|| &wIt is a &G%s&w.  Average Damage: &G%d&w\r\n",
				          ...
				          obj->value[3] == WEAPON_LAUNCHER ? "launcher" :
			  ...
			  if (obj->value[3] == WEAPON_LAUNCHER )
				  ch_printf(ch, "&z|| &wMissiles cartrige size: &G%d\r\n", obj->value[5]);
               ch_printf( ch, "&W++\r\n" );
               break;
