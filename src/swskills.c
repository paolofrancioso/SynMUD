...
void do_makeblade( CHAR_DATA * ch, const char *argument )
{
...

   {
      ...
      xpgain =
         UMIN( obj->cost * 200,
               ( exp_level( ch->skill_level[ENGINEER_ABILITY] + 1 ) -
                 exp_level( ch->skill_level[ENGINEER_ABILITY] ) ) );
...
void do_reinforcements( CHAR_DATA * ch, const char *argument )
{
   ...
   switch ( ch->substate )
   {
      default:
         ...
         if( ch->gold < ch->skill_level[PERSUADER_ABILITY] * 50 )
         {
   ...
   credits = ch->skill_level[PERSUADER_ABILITY] * 50;
...
void do_postguard( CHAR_DATA * ch, const char *argument )
{
   ...
   switch ( ch->substate )
   {
      default:
         ...
         if( ch->gold < ch->skill_level[PERSUADER_ABILITY] * 30 )
         {
   ...
   credits = ch->skill_level[PERSUADER_ABILITY] * 30;
   ch_printf( ch, "It cost you %d credits.\r\n", credits );
   ch->gold -= UMIN( credits, ch->gold );
...
void add_reinforcements( CHAR_DATA * ch )
{
       ...
       ch->backup_mob == MOB_VNUM_NR_FORCES || ch->backup_mob == MOB_VNUM_MERC_FORCES )
   {
      ...
      for( mob_cnt = 0; mob_cnt < max; mob_cnt++ )
      {
         ...
         mob[mob_cnt]->top_level = ch->skill_level[PERSUADER_ABILITY] / 3;
         ...
         mob[mob_cnt]->armor = ( int )( 10 - mob[mob_cnt]->top_level / 1.5 );
   ...
   else
   {
      ...
      mob->top_level = UMIN( ch->skill_level[PERSUADER_ABILITY], 30 );
      for( ability = 0; ability < MAX_ABILITY; ability++ )
...
void do_torture( CHAR_DATA * ch, const char *argument )
{
   ...
   fail = FALSE;
   schance = ris_save( victim, ch->skill_level[PERSUADER_ABILITY], RIS_PARALYSIS );
   if( schance == 1000 )
      fail = TRUE;
   else
                                                                                                       ...
                                                                                                       ACT_BOUND ) ) ) )
   {
      ...
      dam = dice( ch->skill_level[PERSUADER_ABILITY] / 10, 4 );
...
void do_beg( CHAR_DATA * ch, const char *argument )
{
   ...
   percent = number_percent(  ) + ch->skill_level[INFILTRATOR_ABILITY] + victim->top_level;
   ...
   xp =
      UMIN( amount * 10,
            ( exp_level( ch->skill_level[INFILTRATOR_ABILITY] + 1 ) - exp_level( ch->skill_level[INFILTRATOR_ABILITY] ) ) );
   xp = UMIN( xp, xp_compute( ch, victim ) );
   gain_exp( ch, xp, INFILTRATOR_ABILITY );
...
void do_special_forces( CHAR_DATA * ch, const char *argument )
{
   ...
   switch ( ch->substate )
   {
      default:
         ...
         if( ch->gold < ch->skill_level[PERSUADER_ABILITY] * 350 )
         {
            ch_printf( ch, "&RYou dont have enough credits to send for reinforcements.\r\n" );
            return;
         }
   ...
   send_to_char( "&GYour reinforcements are on the way.\r\n", ch );
   credits = ch->skill_level[PERSUADER_ABILITY] * 175;
   ch_printf( ch, "It cost you %d credits.\r\n", credits );
   ch->gold -= UMIN( credits, ch->gold );
...
void do_elite_guard( CHAR_DATA * ch, const char *argument )
{
   ...
   switch ( ch->substate )
   {
      default:
         ...
         if( ch->gold < ch->skill_level[PERSUADER_ABILITY] * 200 )
         {
            ch_printf( ch, "&RYou dont have enough credits.\r\n", ch );
            return;
         }
   ...
   credits = ch->skill_level[PERSUADER_ABILITY] * 200;
   ch_printf( ch, "It cost you %d credits.\r\n", credits );
   ch->gold -= UMIN( credits, ch->gold );
...
void do_smalltalk( CHAR_DATA * ch, const char *argument )
{
   ...
   if( percent - ch->skill_level[PERSUADER_ABILITY] + victim->top_level > ch->pcdata->learned[gsn_smalltalk] )
   {
      /*
       * Failure.
       */
   ...
   gain_exp( ch, victim->top_level * 10, PERSUADER_ABILITY );
...
void do_propeganda( CHAR_DATA * ch, const char *argument )
{
   ...
   gain_exp( ch, victim->top_level * 100, PERSUADER_ABILITY );
...
void do_bribe( CHAR_DATA * ch, const char *argument )
{
   ...
   if( clan == planet->governed_by )
   {
      ...
      amount =
         UMIN( amount,
               ( exp_level( ch->skill_level[PERSUADER_ABILITY] + 1 ) - exp_level( ch->skill_level[PERSUADER_ABILITY] ) ) );

      gain_exp( ch, amount, PERSUADER_ABILITY );
      ch_printf( ch, "You gain %d diplomacy experience.\r\n", amount );
...
void do_mass_propeganda( CHAR_DATA * ch, const char *argument )
{
   ...
   if( number_percent(  ) < ch->pcdata->learned[gsn_masspropeganda] )
   {
      ...
      gain_exp( ch, ch->top_level * 100, PERSUADER_ABILITY );
      ch_printf( ch, "You gain %d diplomacy experience.\r\n", ch->top_level * 100 );
...
void do_repair( CHAR_DATA * ch, const char *argument )
{
...

   {
      ...
      xpgain = ( number_percent(  ) * 6 ) * ch->skill_level[ENGINEER_ABILITY];
      gain_exp( ch, xpgain, ENGINEER_ABILITY );
      ch_printf( ch, "You gain %d engineering experience.\r\n", xpgain );
   }
...
void do_makeduallightsaber( CHAR_DATA * ch, const char *argument )
{
...

   {
      ...
      xpgain =
         UMIN( obj->cost * 50,
               ( exp_level( ch->skill_level[FORCE_ABILITY] + 1 ) - exp_level( ch->skill_level[ENGINEER_ABILITY] ) ) );
      gain_exp( ch, xpgain, FORCE_ABILITY );
      ch_printf( ch, "You gain %d force experience.", xpgain );
   }
...
void do_checkbeacons( CHAR_DATA * ch, const char *argument )
{
   ...
   learn_from_success( ch, gsn_checkbeacons );
   {
      ...
      xpgain = ( ch->experience[MEDIC_ABILITY] / 30 );
      gain_exp( ch, xpgain, MEDIC_ABILITY );
      ch_printf( ch, "You gain %d technician experience.", xpgain );
   }
...
void do_nullifybeacons( CHAR_DATA * ch, const char *argument )
{
   ...
   learn_from_success( ch, gsn_nullifybeacons );
   {
      ...
      xpgain = ( ch->experience[MEDIC_ABILITY] / 25 );
      gain_exp( ch, xpgain, MEDIC_ABILITY );
      ch_printf( ch, "You gain %d technician experience.", xpgain );
   }
