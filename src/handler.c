...
int get_exp_worth( CHAR_DATA * ch )
{
   ...
   wexp = ch->skill_level[COMMANDO_ABILITY] * ch->top_level * 50;
   wexp += ch->max_hit * 2;
   wexp -= ( ch->armor - 5 ) * 2;
...
/* spell cast upon wear/removal of an object	-Thoric */
      ...
      case APPLY_REMOVESPELL:
         ...
         if( IS_VALID_SN( mod ) && ( skill = skill_table[mod] ) != NULL && skill->type == SKILL_SPELL )
            if( ( retcode = ( *skill->spell_fun ) ( mod, ch->skill_level[COMMANDO_ABILITY], ch, ch ) ) == rCHAR_DIED
                || char_died( ch ) )
               return;
         break;
...
void add_kill( CHAR_DATA * ch, CHAR_DATA * mob )
{
   ...
   track = URANGE( 2, ( ( ch->skill_level[COMMANDO_ABILITY] + 3 ) * MAX_KILLTRACK ) / LEVEL_AVATAR, MAX_KILLTRACK );
   for( x = 0; x < track; x++ )
