...
ch_ret multi_hit( CHAR_DATA * ch, CHAR_DATA * victim, int dt )
{
   ...
   {
      dual_bonus = IS_NPC( ch ) ? ( ch->skill_level[COMMANDO_ABILITY] / 10 ) : ( ch->pcdata->learned[gsn_dual_wield] / 10 );
...
short off_shld_lvl( CHAR_DATA * ch, CHAR_DATA * victim )
{
   ...
   {
      lvl = UMAX( 1, ( ch->skill_level[DEFENDER_ABILITY] ) );
      if( number_percent(  ) + ( victim->skill_level[COMMANDO_ABILITY] - lvl ) < 35 )
         return lvl;
      else
         return 0;
   }
   ...
   {
      lvl = ch->top_level;
      if( number_percent(  ) + ( victim->skill_level[COMMANDO_ABILITY] - lvl ) < 70 )
         return lvl;
      else
         return 0;
   }
...
ch_ret one_hit( CHAR_DATA * ch, CHAR_DATA * victim, int dt )
{
   ...
   //if victim is not awake. This is already and better considered in mud.h define GET_AC
   //if( !IS_AWAKE( victim ) )
   //   victim_ac += 5;


   //No more beating up stunned players. But I don't want this.
...
ch_ret damage( CHAR_DATA * ch, CHAR_DATA * victim, int dam, int dt )
{
   ...
   if( dam && ch != victim && !IS_NPC( ch ) && ch->fighting && ch->fighting->xp )
   {
      ...
      gain_exp( ch, xp_gain, COMMANDO_ABILITY );
   }
   ...
   */
   ...
   if( victim->position == POS_DEAD )
   {
       ...
       && new_corpse->in_room == ch->in_room && can_see_obj( ch, new_corpse ) && ch->position > POS_SLEEPING )
      {
         ...
         {
            if( IS_SET( ch->act, PLR_AUTOLOOT ) && IS_NPC( victim ) )
...
void group_gain( CHAR_DATA * ch, CHAR_DATA * victim )
{
   ...
   for( gch = ch->in_room->first_person; gch; gch = gch_next )
   {
      ...
      gain_exp( gch, xp, COMMANDO_ABILITY );

      if( lch == gch && members > 1 )
      {
         xp =
            URANGE( members, xp * members,
                    ( exp_level( gch->skill_level[PERSUADER_ABILITY] + 1 ) -
                      exp_level( gch->skill_level[PERSUADER_ABILITY] ) / 10 ) );
         sprintf( buf, "You get %d leadership experience for leading your group to victory.\r\n", xp );
         send_to_char( buf, gch );
         gain_exp( gch, xp, PERSUADER_ABILITY );
      }
...
int xp_compute( CHAR_DATA * gch, CHAR_DATA * victim )
{
   ...
   xp = ( get_exp_worth( victim )
          * URANGE( 1, ( victim->skill_level[COMMANDO_ABILITY] - gch->skill_level[COMMANDO_ABILITY] ) + 10, 20 ) ) / 10;
   align = gch->alignment - victim->alignment;
   ...
   return URANGE( 1, xp,
                  ( exp_level( gch->skill_level[COMMANDO_ABILITY] + 1 ) - exp_level( gch->skill_level[COMMANDO_ABILITY] ) ) );
}
...
void dam_message( CHAR_DATA * ch, CHAR_DATA * victim, int dam, unsigned int dt )
{
   ...
   if( !dam )
	  ...
	  dampc = dam; //I want flat damage here. Not interested in damaged enemies
      // OLD LOGIC dampc = ( ( dam * 1000 ) / victim->max_hit ) + ( 50 - ( ( victim->hit * 50 ) / victim->max_hit ) );
